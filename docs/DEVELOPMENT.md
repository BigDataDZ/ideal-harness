...
6. **粗粒度模块**：核心 crate 数量冻结在当前规模；新增能力优先放进既有 crate 的子模块。P9 经 D25/TASK-901 批准新增桌面应用边界，并经 TASK-902 新增唯一共享生产装配边界 `harness-host`。

## 2. 目录与依赖规则

见 `AGENTS.md §1` 所有权地图。补充细则：

- 依赖只允许"下游引上游"：{protocol, sandbox-policy, sandbox-exec} ← {network-proxy, tools, session, context} ← agent-loop ← harness-host ← {harness-cli, apps/desktop/src-tauri}
- 生产入口仅允许 `crates/harness-cli` 与 `apps/desktop/src-tauri` 含 `fn main`；二者都必须是薄适配层并共享 `harness-host` 装配，不得复制 agent-loop 或维护第二真相源
