...
| P8 | v0.9 🟡 | **安全与产品化收口**：出网目标钉扎、可取消工具、生产装配、并发一致性、跨平台证据 | 代码与自动化门禁已完成；Windows/Linux CI 与安全回归全绿；发布验收仅待真实模型冒烟（规程就绪，需 key） |
| P9 | v1.0 🟡 | **桌面客户端**：Tauri 2 + React/TypeScript，事件流纯投影，安全审批与跨平台安装包 | 901–909 全部完成：Playwright E2E 与 NSIS 安装包构建在 CI 全绿；用户已安装并启动验证；真实模型冒烟待回填 |
| P10 | v1.1 🚧 | **上游借鉴与多进程收口**：跨进程写租约、ask_user 问答、归因遥测、deny-read 授权入口 | 桌面端与 CLI 并存时无并发写风险；用户可与模型结构化交互；执行可归因；沙箱授权有用户入口 |
...
## 十二、P10 上游借鉴与多进程收口（v1.1，规划）

> 来源：2026-09-05 对 `codex`（150+ 新提交）与 `DeepSeek-Harness`（0.1.3-alpha.1）的增量盘点。
> 核心判断：P9 完成后桌面端与 CLI 并存已成事实，DSH 修复的「双进程写同一会话」正是我们的现存缺口；
> codex 的异步问答与 Windows 沙箱授权 UX 是体验面的下一块拼图。

### TASK-1001: 跨进程会话写租约 ⬜
- 目标 crate: session ⚠️（新增 unix target 依赖决策）、harness-host、apps/desktop/src-tauri
- 内容: 学 DSH 0.1.3 `SessionWriteLease`——桌面端与 CLI 并存时两进程可同时写打开同一会话 JSONL，交错追加会撕坏 seq 与配对。为 JsonlSession 写句柄引入**内核仲裁的写租约**：Windows 命名互斥（kernel32 FFI，与 sandbox-exec 同手法）、POSIX `flock(2)`（libc，unix target）；竞争返回稳定错误；持有者进程死亡由内核自动释放，无 TTL 簿记；未物化会话在首次真实写入前不落任何文件
- 验收标准: 第二进程写打开同一会话返回稳定错误（新 ErrorCode，如 SessionAlreadyOwned）；持有者进程被 kill 后锁由内核释放，后续打开者立即成功；同进程重入与单进程路径零回归；桌面端（Tauri Host）与 CLI 并发写同一会话的集成测试
- 明确不做: 不做 TTL 续约/rename 接管（DSH review 已否决：残余双写竞态窗口）；不做跨机器/网络会话仲裁；不改变 append-only 真相源语义
- 允许依赖: `libc`（session crate 的 unix target，flock 所需）；Windows 使用手动 kernel32 FFI 声明（零依赖）
- 依赖: TASK-805、TASK-903

### TASK-1002: ask_user 结构化问答 ⬜
- 目标 crate: protocol ⚠️串行、agent-loop、harness-cli、apps/desktop
- 内容: 学 codex 异步问答——新增 `UserQuestionRequested` 事件与 `ask_user` 工具：模型给出问题、选项列表（允许多选）与自由输入 Other；CLI 终端呈现选择；桌面端复用审批中心交互框架（事实展示 + 单次应答）；答案作为 tool_result 回流，配对完整
- 验收标准: 问题与答案均经事件留痕可重放；选项选择与 Other 自由输入均可回传；用户取消/超时返回稳定码且不悬挂 turn；答案进入模型表面投影
- 明确不做: 不做语音输入；不做问题模板市场；不做跨 turn 挂起的问答（当前 turn 内收口或取消）
- 依赖: TASK-903

### TASK-1003: 进程生命周期归因遥测 ⬜
- 目标 crate: harness-host、sandbox-exec、agent-loop
- 内容: 学 codex attributed exec telemetry——为受限子进程执行、代理连接与 turn 采样输出**归因化结构日志**（进程 id、耗时分布、退出原因分类、拒绝原因分类），复用既有 Event（ToolResultAdded/NetworkAccessDenied）并补结构化日志通道；敏感信息（命令参数明文、API key）不进入遥测
- 验收标准: 每次受限执行/代理连接/采样均有归因记录；遥测内容不含命令参数明文与密钥；开销可忽略（不引入指标后端依赖）
- 明确不做: 不引入 OTel/指标后端；不做远程上报；不改变现有协议事件
- 依赖: 无（可与 1002 并行）

### TASK-1004: Windows 沙箱 deny-read 授权入口 ⬜
- 目标 crate: sandbox-exec、harness-host、apps/desktop
- 内容: 学 codex managed deny reads + sandbox CLI——把受限 token 的 deny-read 授权暴露为用户可操作入口：桌面工作区页「授予只读目录」（经 DesktopBridge 校验路径后生效）与 CLI `sandbox add-read-dir` 子命令；授权变化推进权限 epoch 与 executor 事实，使旧审批立即失效（603 语义）
- 验收标准: 授权后越界只读可通过且留 Event；epoch 推进使旧审批失效；路径逃逸与符号链接拒绝；桌面端与 CLI 行为一致
- 明确不做: 不做全局白名单；不做永久授权（会话级生效）；不实现 WFP 网络过滤
- 依赖: TASK-603、TASK-903

### P10 建议执行顺序

- **1001 优先**（唯一对应现存真实缺口的卡，涉及 session 依赖决策，先开依赖评审）
- **1002 protocol 串行**；1003、1004 可与之并行
- **P10 出口判据**：桌面端与 CLI 并发写同一会话被租约安全拒绝；ask_user 全链可重放；受限执行与代理有归因遥测；deny-read 授权有桌面与 CLI 入口且旧审批即时失效
...
- **P10 调度**：1001 先行（依赖评审）→ 1002（protocol 串行）；1003、1004 可全程并行
